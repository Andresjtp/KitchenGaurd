...
JWT_SECRET_KEY = os.environ.get('JWT_SECRET_KEY', 'kitchenguard-secret-key-change-in-production')
...
def register_user():
    ...
    try:
        ...
        required_fields = ['username', 'email', 'password', 'fullName', 'restaurantName', 'restaurantType', 'userPosition']
        ...
        kitchen_produce_list = json.dumps(data.get('kitchenProduceList', [])) if data.get('kitchenProduceList') else ''
        bar_produce_list = json.dumps(data.get('barProduceList', [])) if data.get('barProduceList') else ''
        ...
        cursor = conn.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (username, email, password_hash, full_name, restaurant_name, restaurant_type, user_position, kitchen_produce_list, bar_produce_list))
                ...
                try:
                    ...
                    if data.get('kitchenProduceList'):
                        ...
                        if kitchen_response.status_code == 201:
                            ...
                            print(f"⚠️ Kitchen inventory import failed: {kitchen_response.status_code}")
                    ...
                    if data.get('barProduceList'):
                        ...
                        if bar_response.status_code == 201:
                            ...
                            print(f"⚠️ Bar inventory import failed: {bar_response.status_code}")
                except requests.exceptions.RequestException as e:
                    print(f"Warning: Failed to send inventory data to inventory service: {e}")
